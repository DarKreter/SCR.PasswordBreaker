...
// I suggest don't create global or static variables of this class and if you
// do, make sure to call "JoinThreads()" function before main ends. Normally
// destructor will handle joining threads created by this container, but static
// variables may call destructor after mains terminates thread, so it's
...
#define THREAD_WAIT_BEFORE_REMOVAL 500'000	// 500ms
	...
	struct Node_t {
		T value;			  // value
		Node_t *prev, *next;  // pointers to next and previous elements
							  // (bidirectional list)
	...
	pthread_t
		lastRemovalThread;	// information about last launched removal thread
...
public:
	...
	public:
		...
		Iterator(Node_t *_node, SuperiorList *spl = nullptr)
			: currentNode(_node), myList(spl)
		{}
		...
		{
			return this->currentNode == a.currentNode;
		};
		bool operator!=(const Iterator &a) const
		{
			return this->currentNode != a.currentNode;
	...
	SuperiorList()
		: size(0), front(nullptr), back(nullptr),
		  rw_mutex(PTHREAD_RWLOCK_INITIALIZER)
	{}
