...
int main(int argc, char *argv[])
{
	...
	decltype(&pb::WordMod1) WordModTab[3] = {[](string w) { return w; },
											 pb::WordMod1, pb::WordMod2};

reset:
	// First loop divides with combinations and second with dict container
	// third is for word modifications
	for(size_t i = 0, j = combDivide; i < allComb.size();
		i += combDivide, j += combDivide) {
		if(j > allComb.size())
			j = allComb.size();
		for(size_t k = 0, l = dictDivide; k < pb::dict.size();
			k += dictDivide, l += dictDivide) {
			if(l > pb::dict.size())
				l = pb::dict.size();
			for(auto &WordMod: WordModTab)
				// create thread and pass all neccessary args
				pthread_create(
					&breakers[thr_cnt++], &attr, pb::Breaker1,
					reinterpret_cast<void *>(new pb::dataPack(
						pb::dict.begin() + k, pb::dict.begin() + l,
						allComb.begin() + i, allComb.begin() + j, WordMod)));
		}
	}
	...
	while(true) {
		...
		if(!file.is_open())
			...
			for(auto password = pb::passwd.begin();
				password != pb::passwd.end(); ++password)
	...
	if(pb::passwd.length())
		cout << endl
			 << "Breakers generated every password they could! (too bad)"
			 << endl;
	else
